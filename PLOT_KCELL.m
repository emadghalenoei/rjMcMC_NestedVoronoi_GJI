function PLOT_KCELL(timeOFsteps,KCELL,fpath,figname,fignum)
global ExportEPSFig
%%% Plot K
...
subplot(3,2,3)
plot(STEPS,KCELL(:,2),'color','black'),hold on,
% plot(timeOFsteps,mode(NumofNode).*ones(size(NumofNode)),'b--','LineWidth',2)
ylabel('k (salt)')
% xlabel('rjMCMC steps')
xticklabels({})
set(gca,'fontsize',15,'fontweight','bold'),ylim([0 max(KCELL(:,2))+1]),xtickangle(0),ax1 = gca; ax1.XAxis.Exponent = 0;
ax2=axes('Position',ax1.Position,'xlim', [timeOFsteps(1) timeOFsteps(end)], 'color', 'none',...
'YTick',[],'YColor','none','XColor','k', 'XAxisLocation', 'top');
ax2.XAxis.Exponent = 0;
set(ax2,'fontsize',10,'fontweight','bold')
...
subplot(3,2,4)
histogram(KCELL(:,2),5,'Normalization','pdf','Orientation', 'horizontal','DisplayStyle','bar','LineWidth',2,'EdgeColor',[0 0 0],'FaceColor',[0.5 0.5 0.5]), hold on
% plot(linspace(0,0.25,50),mode(NumofNode).*ones(size(linspace(0,0.15,50))),'b--','linewidth',2)
% ylabel('k')
% xlabel('pdf')
set(gca,'fontsize',15,'fontweight','bold'),ylim([0 max(KCELL(:,2))+1])
xtickangle(0),ax1 = gca; ax1.XAxis.Exponent = 0;
box(ax1,'off')
% xlim([0 0.5])

subplot(3,2,5)
plot(STEPS,KCELL(:,3),'color','black'),hold on,
% plot(timeOFsteps,mode(NumofNode).*ones(size(NumofNode)),'b--','LineWidth',2)
ylabel('k (basement)')
xlabel('rjMCMC steps')
set(gca,'fontsize',15,'fontweight','bold'),ylim([0 max(KCELL(:,3))+1]),xtickangle(0),ax1 = gca; ax1.XAxis.Exponent = 0;
ax2=axes('Position',ax1.Position,'xlim', [timeOFsteps(1) timeOFsteps(end)], 'color', 'none',...
'YTick',[],'YColor','none','XColor','k', 'XAxisLocation', 'top');
ax2.XAxis.Exponent = 0;
set(ax2,'fontsize',10,'fontweight','bold')
...
subplot(3,2,6)
histogram(KCELL(:,3),3,'Normalization','pdf','Orientation', 'horizontal','DisplayStyle','bar','LineWidth',2,'EdgeColor',[0 0 0],'FaceColor',[0.5 0.5 0.5]), hold on
% plot(linspace(0,0.25,50),mode(NumofNode).*ones(size(linspace(0,0.15,50))),'b--','linewidth',2)
% ylabel('k')
xlabel('pdf'),
set(gca,'fontsize',15,'fontweight','bold'),ylim([0 max(KCELL(:,3))+1])
xtickangle(0),ax1 = gca; ax1.XAxis.Exponent = 0;
box(ax1,'off')
% xlim([0 0.5])
